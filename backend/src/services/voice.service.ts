...
export const transcribeWithDeepgram = async ({
  ...
  throw new VoiceProviderError(`Deepgram STT failed (${lastFailure?.status || 500})`, {
    ...
    details: (() => {
      ...
      return `Tried Content-Type values: ${mimeCandidates.join(", ")}.
      Last response: ${parsed.message}.${requestLabel}`;
    })()
  });
};
...
// ??$$$ newer code - Whisper STT — proxies to local Python faster-whisper server
