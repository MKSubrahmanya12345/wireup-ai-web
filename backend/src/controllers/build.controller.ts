...
// ??$$$ newer code - compile a scratch Wokwi project or re-run ESP32 simulation analyzer
  ...
  try {
    ...
    if (projectId && req.user?._id) {
      ...
      if (project) {
        ...
        isESP32 = board.includes('esp32') || compute.includes('esp32') || String(fqbn).toLowerCase().includes('esp32');
      }
    }

    // ESP32 simulation bundle re-compilation path
    if (isESP32 && project) {
      ...
      if (session) {
        ...
        const sketch = sketchCode || project.sketch || '';
        const context = session.context || {};
        const name = project.name || session.projectName || 'Hardware Project';

        // Update sketch code in both project and session
        project.sketch = sketch;
        project.markModified('sketch');
        await project.save();

        session.finalSketch = sketch;
        session.markModified('finalSketch');
        await session.save();

        const bundle = compileSimulationBundle(session._id.toString(), {
          name,
          bom,
          wiring,
          milestones,
        ...
        // Emit simulation bundle update to frontend via Socket.io
        const io = (global as any).io;
        if (io) {
          io.to(session._id.toString()).emit("simulation_bundle_update", { bundle });
        }

        return res.json({
          success: true,
          isESP32: true,
          message: "ESP32 compilation re-ran and SimulationBundle was emitted via Socket.io"
        });
      }
    }

    // AVR compilation path (Intel HEX generation)
