...
// ??$$$ newer code - SimulationDriver and LogicRule interfaces
export interface InputControl {
  type: 'slider' | 'button' | 'text_input' | 'file_upload' | 'toggle';
  label: string;
  injectsAs: 'analog_read' | 'digital_read' | 'serial_receive' | 'http_request';
  pin?: string;
  config?: Record<string, any>;
}

export interface OutputBinding {
  type: 'led' | 'canvas' | 'serial_line' | 'audio' | 'gauge';
  triggeredBy: 'digital_write' | 'analog_write' | 'serial_print' | 'display_print';
  pin?: string;
  config?: Record<string, any>;
}
...
export interface LogicRule {
  id: string;
  inputPinOrKey: string;
  operator: '<' | '>' | '==' | '!=' | '<=' | '>=';
  threshold: number | string;
  outputPinOrKey: string;
