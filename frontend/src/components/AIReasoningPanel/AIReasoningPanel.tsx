// ??$$$ newer code - VS Code Copilot Chat exact design component
import {
  ...
  return (
    <div style={{ position: "relative", margin: "8px 0", borderRadius: 4,
      ...
      <div style={{ display: "flex", alignItems: "center",
        padding: "4px 10px", background: "#252526", borderBottom: `1px solid ${V.codeBdr}`, justifyContent: "space-between" }}>
...
export function AIReasoningPanel({
  ...
  model, tokensBurned = 0, onChatInput, onSend, onStop, onNewChat, onModelChange, modelOptions,
}: Props) {
  ...
  const { isRecording, isTranscribing, toggleRecording } = useMicRecorder(
    (transcript: string) => onChatInput(chatInput ? `${chatInput} ${transcript}` : transcript)
  );
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%",
      ...
      <div style={{
        ...
        {/* Token burn counter — visible during and after formulation */}
        {tokensBurned > 0 && (
          <div style={{
            ...
            background: hasActive
              ? "rgba(14,125,212,0.12)"
              : pipelineDone
                ? "rgba(78,201,176,0.10)"
                : "rgba(255,255,255,0.05)",
            border: `1px solid ${hasActive ? "rgba(14,125,212,0.25)" : pipelineDone ? "rgba(78,201,176,0.2)" : "rgba(255,255,255,0.08)"}`,
          }}>
            {/* Flame icon — animated while burning */}
            {hasActive ? (
              <motion.span
            ...
            <motion.span
              ...
              style={{
                ...
                fontFamily: "var(--font-mono)",
                fontWeight: 600,
                color: hasActive ? V.blue : pipelineDone ? V.green : V.textMid,
                letterSpacing: "0.02em",
              }}>
              {tokensBurned >= 1000
                ? `${(tokensBurned / 1000).toFixed(1)}k`
                : tokensBurned.toLocaleString()}
            </motion.span>
            <span style={{ fontSize: 10, color: V.textDim }}>tok</span>
          </div>
        )}
        {/* New chat button */}
