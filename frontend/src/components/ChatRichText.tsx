...
const renderLine = (line, index, isDark) => {
    ...
    <>
      {segments.map((segment, segmentIndex) => (
        <span
          ...
          className={segment.bold ? `rounded px-1 ${isDark ? "bg-white/10 text-white" : "bg-black/8 text-[#1a1a1a]"}` : ""}
        >
          {segment.text}
        </span>
      ))}
  ...
  if (isNumbered) {
    return (
      <div key={`number-${index}`} className={`font-medium ${isDark ? "text-[#f1f1f1]" : "text-[#1a1a1a]"}`}>
        {content}
      </div>
    );
  }
